<div align="center">

# Codex Account Switcher

**Windows 上的 Codex 多账号安全切换器，支持额度查看、设备登录和托盘运行**

![platform](https://img.shields.io/badge/platform-Windows%20x64-0078D4?style=flat)
[![built with](https://img.shields.io/badge/built%20with-.NET%209%20%C2%B7%20WPF-512BD4?style=flat)](https://learn.microsoft.com/dotnet/desktop/wpf/)
[![helper](https://img.shields.io/badge/helper-codex--auth%200.2.10-2EA44F?style=flat)](https://github.com/Loongphy/codex-auth/releases/tag/v0.2.10)

</div>

发布目录中的 `CodexAccountSwitcher.exe` 依赖已安装的 **.NET 9 Desktop Runtime (x64)**。

本项目采用 [MIT License](LICENSE)。内置第三方组件及其来源见
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。

## 使用

1. 运行 `CodexAccountSwitcher.exe`，选择 **Add account** 并确认。工具会先安全关闭 Codex，再启动普通浏览器登录；认证成功的新账号会立即成为活动账号，随后工具重新启动 Codex。登录过程中可以安全取消并恢复此前账号。
2. 选择 **Remove account** 只会打开应用自有的账号列表，不会刷新任何额度。活动账号不能删除，必须先切换到其他账号。
3. 选择 **Refresh** 可立即手动查询额度；应用运行时还会每 2 小时仅刷新当前活动账号，自动请求最多约 12 次/天。界面将已识别的长周期显示为 Weekly 或 Monthly，未知长周期使用中性的 Quota 标签，并显示返回的重置时间；不展示五小时额度。同一额度周期内百分比未变化时不会重复保存采样，重置时间变化后会开始新周期。额度查询使用 unofficial endpoint，可能随服务变化而不可用；手动刷新和账号切换后的立即刷新会产生额外请求，`401`、`403` 或不可用结果不应阻止账号切换。
4. 先停止所有正在进行的 Codex 工作，再选择目标账号并确认。工具会安全关闭 Codex、切换账号、重新启动 Codex，并立即刷新新活动账号的额度。
5. 如果 Codex 侧栏漏显示旧对话，点击顶部的历史按钮，在只读恢复窗口中搜索并用官方 `codex://threads/<会话ID>` 链接重新打开；该功能不会修改会话文件或 Codex 数据库。
6. 点击顶部的统计按钮可查看全部账号在本机产生的 Token 用量，支持今日、最近 7 天、本月和自定义日期范围，并按日拆分输入、缓存输入、输出和总 Token。应用每 30 分钟在本地增量扫描一次 Token，不调用额度接口；缓存输入已包含在输入中，不会重复计入总量。
7. 点击顶部的设置按钮可启用 Windows 开机自启动、选择启动后隐藏到托盘，并在跟随系统、浅色和深色主题之间切换。开机自启动仅写入当前用户的 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`，无需管理员权限。

该工具为单实例应用，同一 Windows 用户只能运行一个实例。工具对 codex-auth 的登录、切换和删除命令设置 `CODEX_AUTH_SKIP_SERVICE_RECONCILE=1`，阻止 codex-auth 创建或修改托管自动切换服务。工具本身不创建计划任务。

不提供 automatic 切换或 hot switch。切换期间不要编辑 Codex 的认证文件，也不要同时运行其他账号工具。单实例只防止本工具的两个实例并发，不会阻止 CCSwitch 等外部程序写入认证文件。切换完成后，不要在 CCSwitch 中启用 **OpenAI Official** provider，否则它可能覆盖当前 Codex 认证状态。

## 安全与恢复

- 账号快照位于 `%USERPROFILE%\.codex\accounts`，当前为未加密存储；不要将该目录或 `.codex\auth.json` 分享、上传或提交到版本库。
- 在首次初始化或真实切换前，备份 `.codex\auth.json` 和 `.codex\accounts`。若账号初始化失败，关闭工具后恢复这份 pre-test `.codex` backup，再重新打开 Codex。
- 点击窗口关闭按钮只会隐藏到 tray，后台额度刷新和 Token 扫描仍会继续；从 tray 菜单选择 Exit 才会退出并停止后台记录。
